...
class User(db.Model):
    ...
    firebase_uid = db.Column('firebase_uid', db.String(128), nullable=False)  # uid for short
...
class UserQuery(object):
    ...
    def get_first_active_user_with_firebase_uid(firebase_uid: str) -> User:
        return User.query.filter(and_(User.firebase_uid == firebase_uid, User.active == true())).order_by(
            desc(User.id)).first()
    ...
    def get_active_users_by_pagination(page: int, per_page: int) -> Pagination:
        return User.query.order_by(User.id.asc()).filter(User.active == true()).paginate(page, per_page,
                                                                                         error_out=False)
